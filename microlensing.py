...
def part_2(graphs=True):
  ...
  get_fit = lambda t, parameters: I_t(t, parameters['umin'], parameters['Tmax'], event.metadata['tau'].value, event.metadata['fbl'].value, event.metadata['I*'].value)
  ...
  bootstrap_predictions = bootstrap(data, lambda data: generate_chi_squared_nd_map(dimensions, data, get_fit, 2)[1], BOOTSTRAP_SAMPLES)
...
def part_3(graphs=True):
  ...
  event = Event(YEAR, ID)
  data = event.data
  umin, Tmax, fbl, tau, I_min = event.metadata['umin'].value, event.metadata['Tmax'].value, event.metadata['fbl'].value, event.metadata['tau'].value, event.metadata['I*'].value
  dimensions = {
    "umin": (umin, 1.5, 20),
    "Tmax": (Tmax, 1.5, 20),
    "fbl": (fbl, 1.5, 20),
  ...
  print('expected values: Tmax:', event.metadata['Tmax'], 'umin:', event.metadata['umin'], 'tau:', event.metadata['tau'], 'fbl:', event.metadata['fbl'],)
  print('1. Generating chi squared map...')
  get_fit = lambda t, parameters: I_t(t, parameters['umin'], parameters['Tmax'], parameters['tau'], parameters['fbl'], I_min)
  chi2_map, best_params = generate_chi_squared_nd_map(dimensions, data, get_fit, 4)

  print('2. Done')

  print(tabulate([[key, event.metadata[key], best_params[key]] for key in dimensions.keys()], headers=['Parameter', 'OGLE', 'Best']))
