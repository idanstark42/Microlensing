...
def part_2(graphs=True):
    ...
    get_fit = lambda t, umin, t0: I_t(t, umin, t0, event.metadata['tau'].value, event.metadata['fbl'].value,
                                      event.metadata['I*'].value)
...
def part_3(graphs=True):
    event = Event(YEAR, ID)
    data = event.data
    umin, Tmax, fbl, tau, I_min = event.metadata['umin'].value, event.metadata['Tmax'].value, event.metadata['fbl'].value, \
                           event.metadata['tau'].value, event.metadata['I*'].value
    dimensions = {
        "umin": (umin, 1.5, 20),
        "Tmax": (Tmax, 1.5, 20),
        "fbl": (tau, 1.5, 20),
    ...
    get_fit = lambda t, _umin, _tmax, _tau, _fbl: I_t(t, _umin, _tmax, _tau, _fbl, I_min)
    chi2_map = generate_chi_squared_nd_map(dimensions, data, get_fit, 4)
