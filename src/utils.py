...
class Value:
  ...
  def __str__(self):
    rounded_error = float(f"{Decimal(f'{self.averror():.2g}'):f}")
    ...
    if type(self.error) == tuple:
      rounded_error = (round_to(self.error[0], decimal_places), round_to(self.error[1], decimal_places))
  ...
  def averror(self):
    if type(self.error) == tuple:
      return (self.error[0] + self.error[1]) / 2
    return self.error
