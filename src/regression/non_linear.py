...
def generate_chi_squared_nd_map(dimensions, data, get_fit, dof):
    ...
    for _ in it:
        if len(dimensions) == 2:
            ...
            fit = get_fit(time, values[it.multi_index]['umin'], values[it.multi_index]['Tmax'],
                          values[it.multi_index]['tau'], values[it.multi_index]['fbl'])
    ...
    plt.plot(time, np.array(list(point['I'].value for point in data)))
    if len(dimensions) == 2:
        plt.plot(time, get_fit(time, values[min_key]['umin'], values[min_key]['Tmax']))
    else:
        plt.plot(time, get_fit(time, values[min_key]['umin'], values[min_key]['Tmax'], values[min_key]['tau'],
                               values[min_key]['fbl']))
    plt.show()
    print(f"Min χ2 = {min_chi2}")
