...
def generate_chi_squared_nd_map(dimensions, data, get_fit, dof):
    ...
    fit = get_fit(time, values[min_ind]['umin'], values[min_ind]['Tmax'], values[min_ind]['tau'],
                  values[min_ind]['fbl'])
    data = np.array(list(point['I'].value for point in data))
    plt.plot(time, data)
    plt.plot(time, fit)
    plt.show()
    ...
    def get_error(key):
        try:
            ...
            lower_index = np.argmin(
                np.array([abs(abs(value['chi2'] - min_chi2) - PPFS[dof][0]) for value in lower_section]))
            upper_index = np.argmin(
                np.array([abs(abs(value['chi2'] - min_chi2) - PPFS[dof][0]) for value in upper_section]))
